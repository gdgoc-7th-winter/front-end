...
import { createBoardPost } from "../api/posts";
      ...
      <form className="grid gap-6 md:gap-8" onSubmit={onSubmit}>
        <div className="grid gap-3">
          ...
          <div className="flex items-center justify-between text-sm text-[#94a3b8]">
            <span>{errors.title?.message ?? submitError ?? "읽기 쉬운 제목으로 핵심 내용을 먼저 적어주세요."}</span>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
            {toolbarButtons.slice(0, 4).map((button) => (
              <ToolbarButton
                ...
                disabled={!editor || createPostMutation.isPending}
                label={button.label}
