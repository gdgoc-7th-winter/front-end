...
import { Bookmark, Eye, Ellipsis, MessageCircle, PencilLine, ThumbsUp, Trash2 } from "lucide-react";
...
export function LectureDetailPage() {
  ...
  const toggleCommentLikeMutation = useMutation({
    ...
    onSuccess: ({ commentId, liked, count }) => {
      setCommentLikeOverrides((previous) => ({
        ...previous,
  ...
  const togglePostLikeMutation = useMutation({
    ...
    onSuccess: async (nextLiked) => {
      const previousLiked =
        postReactionOverride?.liked ?? postDetailQuery.data?.viewer?.liked ?? postDetailQuery.data?.liked ?? postDetailQuery.data?.isLiked ?? false;
      const previousCount = postReactionOverride?.likeCount ?? postDetailQuery.data?.likeCount ?? 0;
  ...
  const togglePostScrapMutation = useMutation({
    ...
    onSuccess: async (nextScrapped) => {
      const previousScrapped =
        postReactionOverride?.scrapped ??
        postDetailQuery.data?.viewer?.scrapped ??
        postDetailQuery.data?.scrapped ??
        postDetailQuery.data?.isScrapped ??
        false;
      const previousCount = postReactionOverride?.scrapCount ?? postDetailQuery.data?.scrapCount ?? 0;
  ...
  const isPostLiked = postReactionOverride?.liked ?? post?.viewer?.liked ?? post?.liked ?? post?.isLiked ?? false;
  const isPostScrapped = postReactionOverride?.scrapped ?? post?.viewer?.scrapped ?? post?.scrapped ?? post?.isScrapped ?? false;
  const postLikeCount = postReactionOverride?.likeCount ?? post?.likeCount ?? 0;
  const postScrapCount = postReactionOverride?.scrapCount ?? post?.scrapCount ?? 0;
                    ...
                    {isActionMenuOpen ? (
                      <div className="absolute right-0 top-12 z-10 w-[220px] rounded-[28px] bg-white p-4 shadow-[0_20px_60px_rgba(15,23,42,0.18)]">
                        <button
                          ...
                          onClick={() => {
                            setIsActionMenuOpen(false);
                            navigate(`/lecture/${numericPostId}/modify`);
                          }}
                        >
