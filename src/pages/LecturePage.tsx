...
const DEPARTMENT_OPTIONS = ["전체 학과", "컴퓨터공학과", "정보통신공학과", "영어통번역학과"] as const;
            ...
            <div className="flex items-center gap-1">
              {BOARD_TABS.map((tab) => {
                ...
                return (
                  <button
                    key={tab.value}
                    className={`relative w-24 px-3 py-[10px] text-base tracking-[-0.03em] ${
          ...
          ? posts.map((post) => (
              <article key={post.postId} className="grid gap-4 py-4">
                <div className="text-xs font-medium text-[#475569]">
                  ...
                  {selectedDepartment !== "전체 학과" ? ` > ${selectedDepartment}` : null}
                </div>

                <div className="flex items-center gap-3">
                  <img alt="" src="/default_profile.png" width={40} height={40} className="rounded-full object-cover" />
                  <div className="grid gap-1">
                    <p className="text-[13px] font-bold leading-5 text-[#0f172a]">{post.authorNickname}</p>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  ...
                  {post.thumbnailUrl ? (
                    <div className="overflow-hidden rounded-[8px] border border-[#e5e7eb] bg-[#111827] md:ml-8 md:w-[220px]">
